...
GetNonPresentingContext() {
  if (!sNonPresentingContext) {
    ...
    sNonPresentingContext = (gvr_context*)nsWindow::CreateGVRNonPresentingContext();
  }
...
mozilla::gfx::SetGVRPresentingContext(void* aGVRPresentingContext)
{
  ...
  if (!sGLContextEGL && aGVRPresentingContext) {
    ...
    nsCString str;
    sGLContextEGL = GLContextEGL::CreateGLContextEGL(flags, caps, nullptr);
    if (!sGLContextEGL) {
      GVR_LOG("Failed to create surfaceless GL context. Fallback to PBuffer based GL context");
  ...
  : VRControllerHost(VRDeviceType::GVR, aHand, aDisplayID)
{
  ...
  mControllerInfo.mNumButtons = GVR_CONTROLLER_BUTTON_COUNT;
...
VRControllerGVR::Update(gvr_controller_state* aState, VRSystemManager* aManager)
{
  ...
  for (int ix = 1; ix < GVR_CONTROLLER_BUTTON_COUNT; ix++) {
    ...
    if (ix == GVR_CONTROLLER_BUTTON_CLICK) {
       ...
       aManager->NewAxisMove(mControllerInfo.mControllerID, 0, xAxis);
       aManager->NewAxisMove(mControllerInfo.mControllerID, 1, yAxis);
    }
    if (pressed) {
    ...
    if (((currentPressMask & buttonMask) ^ (previousPressMask & buttonMask)) ||
      ...
      aManager->NewButtonEvent(mControllerInfo.mControllerID, ix - 1, pressed, touched, pressed ? 1.0 : 0.0);
    }
  }
